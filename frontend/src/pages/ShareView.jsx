...
/* ══════════════════════════════════════════════════════════════
   ROOT — token validation + data fetch
   ══════════════════════════════════════════════════════════════ */
export default function ShareView() {
  ...
  const [selectedInv, setSelectedInv]   = useState(null) // invoice detail drill-down
  ...
  // If viewing an invoice detail, show that instead
  if (selectedInv) return (
    <div style={S.page}>
      <style>{`@keyframes _sv{to{transform:rotate(360deg)}} @keyframes _svbar{from{transform:scaleY(0);transform-origin:bottom}to{transform:scaleY(1);transform-origin:bottom}} ._svbar{animation:_svbar .35s cubic-bezier(.4,0,.2,1) both}`}</style>
      <div style={S.topbar}>
        <div style={S.brand}>
        ...
        <button onClick={() => setSelectedInv(null)} style={S.backBtn}>
          ← Back to Invoices
        </button>
      </div>
      <div style={S.content}>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.topbar}>
        ...
        <div style={{fontSize:11,color:'#9fc8b0',opacity:.7}}>Shared view</div>
      </div>
      <div style={S.content}>
        ...
        {tab === 'invoices'  && (
          <SharedInvoices
            invoices={invoices}
            onSelect={inv => { setSelectedInv(inv); setTab('invoices') }}
          />
        )}
      </div>
    </div>
...
/* ══════════════════════════════════════════════════════════════
   INVOICE DETAIL — full invoice view + PDF download
   ══════════════════════════════════════════════════════════════ */
    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
        ...
        <button
          ...
          style={{display:'inline-flex',alignItems:'center',gap:8,padding:'9px 18px',borderRadius:8,border:'none',cursor:downloading?'default':'pointer',background:'#2d5a45',color:'#fff',fontSize:13,fontWeight:600,opacity:downloading?.75:1,transition:'opacity .12s'}}
        >
          {downloading ? (
      ...
      {/* Invoice preview — scrollable paper */}
      <div style={{background:'#111c14',borderRadius:12,padding:'24px',overflowX:'auto',display:'flex',justifyContent:'center'}}>
...
/* ── Invoice PDF Template (self-contained, no external import) */
    ...
    <div style={{width:794,height:1123,background:'#fff',fontFamily:"'Inter','Helvetica Neue',Arial,sans-serif",color:'#1a2e22',display:'flex',flexDirection:'column',boxSizing:'border-box',overflow:'hidden'}}>
      ...
      <div style={{padding:'0 44px',marginBottom:24}}>
        ...
        <table style={{width:'100%',borderCollapse:'collapse',border:`1px solid ${BORDER}`,borderRadius:6,overflow:'hidden'}}>
          <thead>
            <tr>
              {[['Billing Period (as per timesheet)','left','46%'],['Hours','center','15%'],['Rate / hr','center','20%'],['Amount','right','19%']].map(([h,a,w])=>(
          ...
          <tbody>
            ...
            {lines.length < 3 && Array(3-lines.length).fill(0).map((_,i)=>(
              <tr key={`e${i}`}>
                <td style={{fontSize:12,padding:'11px 14px',borderBottom:`1px solid ${BORDER}`,background:'#fff',color:'transparent'}}>—</td>
                <td style={{padding:'11px 14px',borderBottom:`1px solid ${BORDER}`,background:'#fff'}}/>
                <td style={{padding:'11px 14px',borderBottom:`1px solid ${BORDER}`,background:'#fff'}}/>
                <td style={{padding:'11px 14px',borderBottom:`1px solid ${BORDER}`,background:'#fff'}}/>
              </tr>
            ))}
      ...
      <div style={{background:LIGHT,borderTop:`2px solid ${SAGE}`,padding:'22px 44px',display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:32}}>
        <div>
          ...
          <div style={{fontSize:10,color:'#5a7a6a',lineHeight:1.6}}>Please remit payment within {pmtDays} days of invoice date.{settings?.email?` Questions? Contact ${settings.email}`:''}</div>
        </div>
        <div style={{textAlign:'right'}}>
          ...
          {hstNum && <div style={{fontSize:10,color:'#5a7a6a'}}>{hstNum && `HST# ${hstNum}`}</div>}
        </div>
      </div>
    </div>
  )
...
function SharedInvoices({ invoices, onSelect }) {
  ...
  return (
    <div>
      <div style={{display:'flex',gap:2,borderBottom:'1px solid #e8e2d8',marginBottom:14}}>
        {['all','paid','sent','draft'].map(f=>(
          <button key={f} onClick={()=>setFilter(f)} style={{background:'none',border:'none',borderBottom:`2px solid ${filter===f?'#2d5a45':'transparent'}`,padding:'8px 14px',marginBottom:-1,cursor:'pointer',fontSize:13,fontWeight:500,fontFamily:'Inter,sans-serif',color:filter===f?'#2d5a45':'#9a9080'}}>
      ...
      <div style={{background:'#fff',border:'1px solid #e8e2d8',borderRadius:12,overflow:'hidden'}}>
        ...
        {visible.length===0
          ...
          : visible.map((inv,i)=>(
            <div key={inv.id} style={{display:'grid',gridTemplateColumns:'130px 1fr 110px 120px 80px 80px',gap:8,padding:'11px 18px',borderBottom:i<visible.length-1?'1px solid #f5f2ec':'none',alignItems:'center',cursor:'pointer',transition:'background .1s'}}
      ...
      <div style={{marginTop:20,textAlign:'center',fontSize:11,color:'#c0b8ac'}}>Click any invoice to view details and download PDF · Read-only shared view</div>
    </div>
  )
}

/* ══════════════════════════════════════════════════════════════
   SHARED DASHBOARD (unchanged from before)
   ══════════════════════════════════════════════════════════════ */
function SharedDashboard({ invoices, clients, settings }) {
  ...
  const yearInv = invoices.filter(i=>new Date(i.invoice_date+'T12:00:00').getFullYear()===selYear)
  const ytdRev  = yearInv.reduce((s,i)=>s+(+i.subtotal),0)
  const ytdHST  = yearInv.filter(i=>i.status!=='draft').reduce((s,i)=>s+(+i.hst_amount),0)
  const ytdPaid = yearInv.filter(i=>i.status==='paid').reduce((s,i)=>s+(+i.total),0)
  const ytdOut  = yearInv.filter(i=>i.status==='sent').reduce((s,i)=>s+(+i.total),0)
  const ytdPN   = yearInv.filter(i=>i.status==='paid').length

  const monthly = Array(12).fill(0)
  yearInv.forEach(i=>{monthly[new Date(i.invoice_date+'T12:00:00').getMonth()]+=+i.subtotal})
  const maxBar = Math.max(...monthly,1)
  const years  = [...new Set(invoices.map(i=>new Date(i.invoice_date+'T12:00:00').getFullYear()))].sort((a,b)=>b-a)

  const now=new Date(), todayMD=now.getMonth()*100+now.getDate()
  const yoyData=[...years].reverse().slice(-4).map(y=>{
    const inv=invoices.filter(i=>{const d=new Date(i.invoice_date+'T12:00:00');return d.getFullYear()===y&&(d.getMonth()*100+d.getDate())<=todayMD})
    return{year:y,rev:inv.reduce((s,i)=>s+(+i.subtotal),0),count:inv.length,isCur:y===curYear}
  })
  ...
  if(yoyData.length>=2){
    ...
    growth=(<div style={{marginTop:14,padding:'10px 14px',background:up?'#f0faf4':'#fdf5ec',borderRadius:8,border:`1px solid ${up?'#c8e6d4':'#f5d8a8'}`}}>
      ...
      <div style={{fontSize:11,color:'#7a8070'}}>{up?`+${fmt(lat.rev-prv.rev)} ahead of ${lbl} last year`:`${fmt(prv.rev-lat.rev)} behind ${lbl} last year`}</div>
    </div>)
  }
  ...
  return (
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:14}}>
        {[{label:'Lifetime Revenue',val:fmt(lifetimeRev),color:'#fff',dark:true,sub:`${invoices.length} invoices`},{label:'HST Collected',val:fmt(lifetimeHST),color:'#c07820',sub:'all time'},{label:'Total Paid',val:fmt(lifetimePaid),color:'#2d5a45',sub:`${paidInv.length} invoices`},{label:'Avg Invoice',val:fmt(avgInvoice),color:'#5a4a3a',sub:'before HST'}].map((k,i)=>(
          <div key={i} style={{...C,...(k.dark?{background:'#2d5a45',border:'none'}:{})}}>
            ...
            <div style={{fontFamily:"'Sora',sans-serif",fontSize:22,fontWeight:800,color:k.color,lineHeight:1}}>{k.val}</div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 260px',gap:14,marginBottom:14}}>
        <div style={C}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:16}}>
            ...
            <div style={{display:'flex',gap:3,background:'#f5f2ec',borderRadius:8,padding:3}}>
              {years.map(y=>(
                <button key={y} onClick={()=>{setSelYear(y);setAnimKey(k=>k+1)}} style={{fontSize:11,fontWeight:600,padding:'4px 10px',borderRadius:6,border:'none',cursor:'pointer',background:selYear===y?'#fff':'none',color:selYear===y?'#2d5a45':'#7a8070',boxShadow:selYear===y?'0 1px 4px rgba(0,0,0,.08)':'none'}}>{y}</button>
              ))}
            </div>
          </div>
          <div style={{display:'flex',alignItems:'flex-end',gap:5,height:90}} key={animKey}>
            {monthly.map((val,i)=>{
              const isAct=selYear===curYear&&i===curMon,isFut=selYear===curYear&&i>curMon
              return(<div key={i} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
                <div style={{width:'100%',display:'flex',alignItems:'flex-end',height:72}}>
                  <div className="_svbar" style={{width:'100%',borderRadius:'4px 4px 0 0',minHeight:3,height:`${Math.max(3,(val/maxBar)*72)}px`,background:isAct?'#2d5a45':isFut?'#e8e4dc':'#9fc8b0',animationDelay:`${i*30}ms`}}/>
                </div>
                <div style={{fontSize:9,color:'#b0a898',fontWeight:600}}>{MONTHS[i]}</div>
                {val>0&&<div style={{fontSize:8,color:'#7a8070'}}>{fmtK(val)}</div>}
              </div>)
            })}
          </div>
        </div>
        <div style={C}>
          ...
          {[['Invoices',`${yearInv.length} total`,''],['Paid',`${ytdPN} invoices`,''],['Net Revenue',fmt(ytdRev),'#2d5a45'],[`HST (${hstRate}%)`,fmt(ytdHST),'#c07820'],['Outstanding',fmt(ytdOut),ytdOut>0?'#c07820':'#9a9080'],['Collected',fmt(ytdPaid),'#2d5a45']].map(([l,v,c])=>(
          ...
          {yoyData.map((d,i)=>(
            <div key={d.year} style={{display:'flex',alignItems:'center',gap:10,marginBottom:i<yoyData.length-1?12:0}}>
              ...
              <div style={{flex:1,height:6,background:'#f0ece4',borderRadius:3,overflow:'hidden'}}>
                <div style={{height:'100%',borderRadius:3,width:`${(d.rev/maxYoy)*100}%`,background:d.isCur?'#2d5a45':`rgba(45,90,69,${0.3+i*0.18})`}}/>
              </div>
              <div style={{fontSize:11,fontWeight:700,color:'#1a2e22',width:60,textAlign:'right',fontVariantNumeric:'tabular-nums'}}>{fmtK(d.rev)}</div>
              <div style={{fontSize:10,color:'#b0a898',width:24,textAlign:'right'}}>{d.count}×</div>
            </div>
          ))}
          {growth}
        </div>
...
const C = {background:'#fff',border:'1px solid #e8e2d8',borderRadius:12,padding:'20px 22px'}
const S = {
  ...
  errorBox:{background:'#fff',border:'1px solid #e8e2d8',borderRadius:12,padding:'32px 40px',textAlign:'center',maxWidth:360},
  topbar:  {background:'#2d5a45',padding:'14px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:16},
  brand:   {display:'flex',alignItems:'center',gap:10},
  brandDot:{width:32,height:32,background:'rgba(255,255,255,.15)',borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Sora',sans-serif",fontSize:14,fontWeight:700,color:'#fff'},
  ...
  tab:     {fontSize:12,fontWeight:600,color:'rgba(255,255,255,.6)',padding:'5px 14px',borderRadius:6,border:'none',background:'none',cursor:'pointer',transition:'all .12s'},
  tabActive:{background:'rgba(255,255,255,.2)',color:'#fff'},
  backBtn: {display:'inline-flex',alignItems:'center',gap:6,fontSize:12,fontWeight:600,color:'#9fc8b0',background:'rgba(255,255,255,.1)',border:'none',borderRadius:6,padding:'6px 12px',cursor:'pointer'},
  content: {maxWidth:1100,margin:'0 auto',padding:'24px 24px 48px'},
}
