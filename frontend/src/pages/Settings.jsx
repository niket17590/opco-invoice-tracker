...
export default function Settings() {
  ...
  const [form, setForm] = useState({
    ...
    invoice_prefix:'INV-', hst_rate:'13.00',
  })
  ...
  async function saveSharing(enabled, pages) {
    setSavingShare(true)
    await supabase.from('settings').update({
      sharing_enabled: enabled, shared_pages: pages,
      updated_at: new Date().toISOString(),
    }).eq('user_id', user.id)
  ...
  function togglePage(p) {
    const n = sharedPages.includes(p) ? sharedPages.filter(x=>x!==p) : [...sharedPages, p]
    setSharedPages(n); saveSharing(sharingEnabled, n)
  }
  function copyLink() {
    navigator.clipboard.writeText(`${APP_URL}/share/${shareToken}`)
      ...
      <div className="st-grid">
        ...
        <div className="st-col">
          ...
          <div className="settings-card" style={{margin:0}}>
            ...
            <p className="fs-11 text-muted mt-4">Sign out via the sidebar (desktop) or Settings menu.</p>
          </div>
        </div>
        ...
        <div className="st-col">
          ...
          <div className="settings-card" style={{margin:0}}>
            <div className="settings-card-title">
              ...
              <span className="settings-badge" style={{
                color:      sharingEnabled ? 'var(--green-text)' : 'inherit',
                background: sharingEnabled ? 'var(--green-pale)' : 'var(--linen)',
                border:     `1px solid ${sharingEnabled ? 'var(--green-border)' : 'var(--border)'}`,
              }}>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:14}}>
              ...
              <button
                ...
                style={{width:40,height:22,borderRadius:11,border:'none',cursor:'pointer',background:sharingEnabled?'var(--sage-dark)':'var(--linen-dark)',position:'relative',transition:'background .2s',flexShrink:0,marginLeft:12}}
              >
            ...
            <div style={{display:'flex',gap:8,marginBottom:14}}>
              {[{key:'dashboard',label:'Dashboard',desc:'Charts & stats'},{key:'invoices',label:'Invoices',desc:'Invoice history'}].map(p => {
                ...
                return (
                  <button key={p.key} onClick={() => togglePage(p.key)} disabled={savingShare}
                    style={{flex:1,padding:'10px 12px',borderRadius:8,cursor:'pointer',textAlign:'left',border:`2px solid ${on?'var(--sage-dark)':'var(--border)'}`,background:on?'var(--sage-pale)':'var(--white)',transition:'all .15s'}}>
            ...
            {shareToken && (
              <div>
                ...
                <p style={{fontSize:11,color:'var(--text-muted)',marginTop:5}}>
                  {!sharingEnabled ? 'Enable sharing to activate this link.'
                    : sharedPages.length===0 ? 'Select at least one page.'
                    : `Sharing ${sharedPages.join(' & ')} in read-only mode.`}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="settings-card" style={{margin:0}}>
            <div className="settings-card-title">
              ...
              <span className="settings-badge" style={{
                color:      driveConnected ? 'var(--green-text)' : 'inherit',
                background: driveConnected ? 'var(--green-pale)' : 'var(--linen)',
                border:     `1px solid ${driveConnected ? 'var(--green-border)' : 'var(--border)'}`,
              }}>
            ...
            {driveConnected ? (
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 12px',background:'var(--green-pale)',border:'1px solid var(--green-border)',borderRadius:'var(--radius-md)',marginBottom:14,gap:8,flexWrap:'wrap'}}>
                ...
                <button onClick={handleDisconnectDrive} disabled={disconnecting}
                  style={{fontSize:11,color:'var(--text-muted)',background:'none',border:'none',cursor:'pointer',padding:'4px 8px',borderRadius:4,fontFamily:'Inter,sans-serif'}}>
              ...
              <div style={{marginBottom:14}}>
                <button
                  ...
                  style={{display:'flex',alignItems:'center',gap:10,width:'100%',padding:'10px 14px',borderRadius:'var(--radius-md)',border:'1.5px solid var(--border)',background:'var(--white)',cursor:connectingDrive?'default':'pointer',fontSize:13,fontWeight:500,fontFamily:'Inter,sans-serif',color:'var(--text-primary)',opacity:connectingDrive?.7:1}}
            ...
            <div>
              ...
              {driveFolderId ? (
                <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'9px 12px',background:'var(--linen-mid)',border:'1px solid var(--border)',borderRadius:'var(--radius-md)',gap:8,flexWrap:'wrap'}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,minWidth:0}}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="#2d5a45" style={{flexShrink:0}}><path d="M10 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/></svg>
                    <div style={{minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:600,color:'var(--text-primary)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{folderName || 'Selected folder'}</div>
                      <div style={{fontSize:10,color:'var(--text-muted)',fontFamily:'monospace'}}>{driveFolderId.slice(0,20)}…</div>
                    </div>
                  </div>
                  <button onClick={openFolderPicker} disabled={!driveConnected||pickingFolder}
                    style={{fontSize:11,color:'var(--sage-dark)',background:'none',border:'none',cursor:'pointer',fontWeight:600,fontFamily:'Inter,sans-serif',flexShrink:0}}>
                    Change
                  </button>
                </div>
              ) : (
...
function DriveIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 87.3 78" fill="none" style={{flexShrink:0}}>
